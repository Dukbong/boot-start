...
//	@PostMapping("/login")
	public String loginV3(@Validated @ModelAttribute LoginForm form, BindingResult bindingResult, HttpServletRequest request) {
		...
		// getSession(true) : 세션이 있으면 기존 세션 반환 / 없으면 새로운 세션 생성후 반환
		// >> true가 기본값이다 / 생략 가능
		// getSession(false) : 세션이 있으면 기존 세션 반환 / 없으면 null 반환
...
//	@PostMapping("/logout")
	...
	public String logoutV3(HttpServletRequest request) {
		...
		if(session != null) {
			session.invalidate(); // 세션 삭제 하는 메소드
		}
		sessionManager.expire(request);
		return "redirect:/";
	}
	...
	private void expireCookie(HttpServletResponse response, String cookieName) {
		// 제거할 쿠키와 같은 이름의 쿠키를 만든 후 잔여시간을 0으로 만들고 클라이언트에게 보낸다.
		// - 쿠키는 Map과 같이 key의 중복이 되지 않고 덮어쓴다.
