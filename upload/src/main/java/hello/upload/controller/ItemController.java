...
public class ItemController {
	...
	public String saveItem(@ModelAttribute ItemForm form, RedirectAttributes redirectAttributes) throws IllegalStateException, IOException {
		MultipartFile attach = form.getAttachFile();
		UploadFile attachFile = fileStore.storeFile(attach);
	...
	public ResponseEntity<Resource> downloadAttach(@PathVariable Long itemId) throws MalformedURLException{
		...
		String uplaodFileName = item.getAttachFile().getUploadFileName();
		
		UrlResource resouce = new UrlResource("file:"+fileStore.getFullPath(storeFileName));
		
		String encodedUploadFileName = UriUtils.encode(uplaodFileName, StandardCharsets.UTF_8); 
		String contentDisposition = "attachment; filename=\"" + encodedUploadFileName + "\"";
		
		log.info("uploadFIleName = {}", uplaodFileName);
